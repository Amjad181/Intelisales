...
Use this checklist before merging backend handoff changes into `main` and before frontend/mobile demo recording.
...
- [ ] `git status` shows only intended `Back-End` changes.
...
- [ ] Pull request targets `main` and is not auto-merged.
