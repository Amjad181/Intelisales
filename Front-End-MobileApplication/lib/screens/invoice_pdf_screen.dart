...
class InvoicePdfScreen extends StatefulWidget {
  ...
  /// معرّف الفاتورة الحقيقي من الباك اند إن كانت محفوظة مسبقاً — عندها
  /// نجلب الـPDF الرسمي من الخادم بدل توليده محلياً.
...
class _InvoicePdfScreenState extends State<InvoicePdfScreen> {
  ...
  bool _saved = false;
  late final String? _invoiceId = widget.invoiceId;
  ...
  Future<void> _confirmAndSave(BuildContext context, bool ar) async {
    final invoiceId = _invoiceId;
    if (invoiceId == null) {
      // معاينة غير محفوظة تصل هنا فقط في وضع العرض التجريبي الصريح —
      // وضع التكامل لا يفتح هذه الشاشة أصلاً دون مسودة حقيقية.
      setState(() => _saved = true);
      _showSnack(
        context,
        ar
            ? 'تم التأكيد محلياً فقط (وضع تجريبي) — لم يتم الحفظ على الخادم'
            : 'Confirmed locally only (demo mode) — not saved to the server',
      );
      return;
    }
    try {
      await InvoiceService.confirm(invoiceId);
      if (!context.mounted) return;
      setState(() => _saved = true);
      _showSnack(
        context,
        ar ? 'تم تأكيد وحفظ الفاتورة' : 'Invoice confirmed and saved',
      );
  ...
  /// فاتورة محفوظة → PDF الرسمي من الخادم حصراً (bytes ثنائية حسب العقد)؛
  /// فشل الجلب يظهر كخطأ حقيقي دون توليد محلي بديل. التوليد المحلي متاح
  /// فقط للمعاينات غير المحفوظة في وضع العرض التجريبي الصريح.
  Future<Uint8List> _generatePdfBytes(bool ar) async {
    final invoiceId = _invoiceId;
    if (invoiceId != null) {
      return InvoiceService.fetchPdfBytes(invoiceId);
    }
  ...
  Future<void> _exportPdf(bool ar) async {
    Uint8List bytes;
    try {
      bytes = await _generatePdfBytes(ar);
    } on ApiException catch (e) {
      if (mounted) _showSnack(context, e.message);
      return;
    }
    if (!mounted) return;
                      ...
                      PopupMenuItem(
                        ...
                        child: Row(
                          children: [
                            ...
                            const SizedBox(width: 10),
                            Text(
                              ar ? 'مشاركة على واتساب' : 'Share via WhatsApp',
                            ),
                          ],
                        ),
                      ),
          ...
          children: [
            ...
            Container(
              ...
              child: Row(
                children: [
                  Icon(
                    _saved ? Icons.verified : Icons.edit_document,
            ...
            children: [
              SizedBox(
                ...
                child: ElevatedButton.icon(
                  onPressed: _saved ? null : () => _confirmAndSave(context, ar),
                  icon: Icon(
